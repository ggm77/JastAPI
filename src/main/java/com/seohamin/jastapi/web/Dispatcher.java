 ...
 * A class responsible for dispatching incoming external requests to the appropriate handlers.
 */
@Component
public class Dispatcher {
     ...
     * Processes the incoming HTTP request, dispatches it to the corresponding handler,
     * and returns the appropriate HTTP response.
     * @param httpRequest Http 요청 정보가 담긴 객체 (The object containing HTTP request information.)
     * @return Http 응답 값 (The resulting HTTP response.)
     */
     ...
     * Converts an HTTP method string into its corresponding Enum constant.
     * @param methodStr 문자열로 된 HTTP method. (The HTTP method as a string.)
     * @return 적절한 HttpMethod 객체 (The matching HttpMethod object.)
     */
     ...
     * Invokes the method stored in the route and generates an HTTP response by processing the return value.
     * @param routeInfo 요청에 해당하는 라우트 정보가 담긴 객체. (The object containing the route details for the request.)
     * @param args invoke할 객체의 매개변수 배열 (The array of arguments for the method invocation.)
     * @param httpRequest HTTP 요청 (The original HTTP request.)
     * @return 적절하게 생성된 HTTP 응답 (The constructed HTTP response.)
     * @throws InvocationTargetException invoke한 메서드에서 예외가 터진 경우 던짐. (If the invoked method throws an exception.)
     * @throws IllegalAccessException invoke 할 때 매개변수가 적절하지 않다면 던짐. (If the method cannot be accessed or arguments are invalid.)
     */
    private HttpResponse invokeAndCreateResponse(
        ...
        // 리플랙션으로 라우터에서 찾은 메소드  | Execute the method found in the router using reflection
        final Object result = routeInfo.getMethod().invoke(routeInfo.getInstance(), args);

        // 메서드 호출 결과가 HttpResponse인 경우 캐스팅해서 바로 전송 | If the result is already an HttpResponse, cast and return it
        if (result instanceof HttpResponse) {
            return (HttpResponse) result;
        } else {
            // 바디를 byte로 변환 | Convert the body to a byte array
            final byte[] body = Converter.convertToByte(result);

            // http response 객체 필드 채우기 | Populate the fields of the HttpResponse object
     ...
     * Parses the HTTP request to extract and resolve arguments required for method invocation.
     * @param routeDto invoke 할 메서드 정보가 담긴 객체 (The object containing information about the method to be invoked.)
     * @param httpRequest HTTP 요청 정보 (The HTTP request information.)
     * @return 매개변수가 담긴 배열 (An array containing the resolved arguments.)
     * @throws IOException HTTP 요청 바디를 읽기 실패한 경우 예외 던짐. (If an error occurs while reading the request body.)
     */
    private Object[] resolveArguments(
            final RouteDto routeDto,
            final HttpRequest httpRequest
    ) throws IOException {
        // 실제 객체와 메소드 정보, 파라미터 정보가 담긴 객체 추출
        // Extract actual object, method, and parameter info
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            // Parameter is a RequestParam
            else if (parameter.getParameterSource().equals(ParameterSource.PARAM)) {
                // 쿼리에 키만 적혀있고, 값은 없는 경우
                // Handle cases where the key exists but the value is missing in the query
                if (query == null) {
                    // 빈 리스트 반환 | return empty list
                    args[i] = Collections.emptyList();
                }
                else {
                    // 요소가 몇개든 List<String>으로 반환
                    // Return as List<String> regardless of the number of elements
                    args[i] = query.get(parameter.getAnnotationValue());
                }
            }
            // 해당 파라미터의 타입이 HttpRequest인 경우
            // Parameter type is HttpRequest
     ...
     * 이외에는 500을 던진다.
     * Handles exceptions thrown from within the invoked method.
     * If it is a developer-intended HttpResponseException, it returns the appropriate response;
     * otherwise, it returns a 500 Internal Server Error.
     * @param ex invoke 한 메서드 내에서 터진 InvocationTargetException. (The InvocationTargetException thrown during reflection.)
     * @param version 응답할 HTTP 버전. (The HTTP version for the response.)
     * @return 적절한 HttpResponse 또는 500. (The appropriate HttpResponse or a 500 error response.)
     */
    private HttpResponse handleInvocationException(
        ...
        // If the HttpResponseException was intentionally caused by the developer, return an appropriate response.
