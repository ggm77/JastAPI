...
Add the JitPack repository. If your project uses `dependencyResolutionManagement` in `settings.gradle` (the default for projects generated by recent Gradle/IntelliJ versions), add it there:
```gradle
// settings.gradle
...
```
Otherwise, add it to your `build.gradle`'s `repositories` block instead.
...
```
`Jackson Databind` is included transitively, so no separate dependency is needed.
